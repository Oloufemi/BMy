...
let quotes = [
  ...
  let otherQuoteButton = document.querySelector('#anotherQuote');

  function random(max) {
    return Math.floor(Math.random() * Math.floor(max));
  }

  function showOtherQuote() {
    getIndex();
    quoteDiv.textContent = quotes[randomValue][0];
    authorDiv.textContent = quotes[randomValue][1];
    lastChoosedQuote = randomValue;
  }

  function getIndex() {
    do {
    randomValue = random(quotes.length - 1);
    } while( randomValue === lastChoosedQuote)
  }

  // Variables
  let lastChoosedQuote = 0;
  let randomValue = 0;
  
  otherQuoteButton.addEventListener('click', showOtherQuote);